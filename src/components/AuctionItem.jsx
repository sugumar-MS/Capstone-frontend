...
function AuctionItem() {
	...
	useEffect(() => {
		...
		const fetchWinner = async () => {
			try {
				const res = await axios.get(`https://capstone-backend-se7v.onrender.com/api/auctions/winner/${id}`);
	...
	useEffect(() => {
		const fetchBids = async () => {
			setLoadingBids(true);
			try {
				const res = await axios.get(`https://capstone-backend-se7v.onrender.com/api/bids/${id}`);
	...
	const handleDelete = async () => {
		try {
			await axios.delete(`https://capstone-backend-se7v.onrender.com/api/auctions/${id}`);
			navigate("/auctions");
		} catch (error) {
