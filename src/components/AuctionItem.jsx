...
function AuctionItem() {
	...
	useEffect(() => {
		...
		const fetchWinner = async () => {
			try {
				const res = await axios.get(`https://capstone-backend-1-q22o.onrender.com/api/auctions/winner/${id}`);
	...
	useEffect(() => {
		const fetchBids = async () => {
			setLoadingBids(true);
			try {
				const res = await axios.get(`https://capstone-backend-1-q22o.onrender.com/api/bids/${id}`);
	...
	const handleDelete = async () => {
		try {
			await axios.delete(`https://capstone-backend-1-q22o.onrender.com/api/auctions/${id}`);
			navigate("/auctions");
		} catch (error) {
